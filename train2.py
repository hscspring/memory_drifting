...
def train_one_dialogue(model, batch, optimizer):
    m_prev = model.m_0.clone().detach()
    total_dialogue_loss = 0.0
    ...
    for idx, turn in enumerate(batch["dialogue_turns"]):
        ...
        if update_flag.item() > 0.5:
            ...
            with torch.no_grad():
                ...
                e_negative = model.base_model.model(negative_state_ids).last_hidden_state[:, -1, :].detach()
                assert e_negative.dim() == 2
        else:
            slot_label = turn["slot_label"].to(model.device)
            offset_label = turn["relative_offset"].to(device)
            e_target = None
            e_negative = None
        ...
        with autocast(device_type="cuda", dtype=torch.bfloat16):
            loss_dict, m_new, g_logits = model(
                ...
                update_flag, e_target, e_negative, offset_label
            )
            ...
            loss = llm_loss + 3.0 * target_loss + 0.5 * offset_loss + 0.01 * probe_loss
    ...
    losses = {
        ...
        "offset_loss": sum([v["offset_loss"] for v in loss_lst])/len(loss_lst) if loss_lst else 0,
    }
...
def train():
    ...
    data_path = "mocker/mock_dialogues_multi_domain.json"
